...
export function getProducts(myCallBack) {
  const productrewetbody = {
    advancedSearch: {
      fields: ['id'],
      keyword: ''
    },
  ...
  return async (dispatch, getState) => {
    ...
    try {
      const response = await axios.post('/v1/products/search', productrewetbody);
...
export function postProduct(newProduct, myCallBack) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.post('/v1/products', newProduct);
      dispatch(slice.actions.postProductSuccess());
